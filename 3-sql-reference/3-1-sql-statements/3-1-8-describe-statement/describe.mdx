...
<Check>The `Describe` statement is recommended to use before creating a new table to avoid duplicate tables.</Check>
...
<Check>The example above shows that the tables reside in the `public` schema, the default schema in Oxla. You can also have a table from other schemas - click [here](https://oxla.mintlify.app/3-sql-reference/schema) for more info.</Check>
