...
<Info>Oxla also has the `REGEXP_REPLACE()` function. It enables you to search and replace a substring(s) that matches with a POSIX regular expression. See [here](https://docs.oxla.com/regexpreplace) for more info.</Info>
